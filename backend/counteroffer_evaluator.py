...
def detect_deadlock(history: list, n_rounds: int = 3) -> bool:
    """Detect whether the negotiation has stalled across the last N rounds.

    A round is considered stalled when the numeric offers made during the
    recent completed rounds vary by less than 1% of their mean. This keeps
    the detector tied to negotiation rounds rather than raw message count.
    """
    if n_rounds <= 0:
        return False
    ...
    recent_rounds = set(rounds)

    recent_offers = [
        float(entry["offer"])
        for entry in history
        if entry.get("round") in recent_rounds
        and entry.get("offer") is not None
    ]
    ...
    return (spread / abs(mean_offer)) < 0.01
